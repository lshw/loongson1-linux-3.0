...
static void dma_enable_trans(audio_stream_t * s,audio_dmadesc_t *desc)
{
	...
	val = desc->snd_dma_handle;//第一个DMA描述符的地址
	val |= s->output ? 0x9 : 0xa;
	local_irq_save(flags);
	write_reg(CONFREG_BASE + 0x1160,val);
	udelay(1000);
	while(read_reg(CONFREG_BASE + 0x1160)&0x8) ;
...
//		sb2f_codec_write(NULL,0x32,sample_rate);//|(0x32<<16)|(0<<31));     //pcm input rate .
	...
	//	write_reg(OCCR,(os->fmt == AFMT_S16_LE)?0x6b6b:0x6363); //OCCR0   L&& R enable ; 3/4 empty; dma enabled;8 bits;var rate(0x202);
		write_reg(OCCR,(os->fmt == AFMT_S16_LE)?0x6969:0x6161); //OCCR0   L&& R enable ; 3/4 empty; dma enabled;8 bits;var rate(0x202);
...
static int sb2f_audio_probe(struct platform_device *pdev)
{
	...
	if (!sb2f_ac97_refcount) {
		...
		{
			if(sb2f_ac97_codec.model == 0x414c4760) {capmode = 0x00001;}
