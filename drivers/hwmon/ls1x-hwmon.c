...
static unsigned int ls1x_adc_read(struct ls1x_hwmon *hwmon, unsigned int ch)
{
	...
	if (hwmon->single) {
		...
		switch (ch) {
		case 0:
			while (readl(hwmon->regs + ADC_S_DOUT0) & 0x80000000) {
			}
		...
		case 1:
			...
			break;
		case 2:
			while (readl(hwmon->regs + ADC_S_DOUT1) & 0x80000000) {
			}
...
*/
static int __devinit ls1x_hwmon_probe(struct platform_device *dev)
{
...
static int __devexit ls1x_hwmon_remove(struct platform_device *dev)
{
...
static struct platform_driver ls1x_hwmon_driver = {
	...
	.remove		= __devexit_p(ls1x_hwmon_remove),
