...
FTS_BOOL i2c_read_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	if(ret<=0){
		dprintk("[TSP]i2c_read_interface error\n");
		return FTS_FALSE;
	}
...
FTS_BOOL i2c_write_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	if(ret<=0){
		dprintk("[TSP]i2c_write_interface error line = %d, ret = %d\n", __LINE__, ret);
		return FTS_FALSE;
	}
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    dprintk("[TSP] Step 1: Reset CTPM test\n");
    ...
    {
        dprintk("[TSP] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
    }
    else
    {
        return ERR_READID;
    ...
    delay_qt_ms(1500);
    dprintk("[TSP] Step 4: erase. \n");

    /*********Step 5:write firmware(FW) to ctpm flash*********/
    bt_ecc = 0;
    dprintk("[TSP] Step 5: start upgrade. \n");
    dw_lenth = dw_lenth - 8;
    ...
    for (j=0;j<packet_number;j++)
    {
        ...
        {
              dprintk("[TSP] upgrade the 0x%x th byte.\n", ((unsigned int)j) * FTS_PACKET_LENGTH);
        }
    }
    ...
    byte_read(reg_val,1);
    dprintk("[TSP] Step 6:  ecc read 0x%x, new firmware 0x%x. \n", reg_val[0], bt_ecc);
    if(reg_val[0] != bt_ecc)
    {
        return ERR_ECC;
    }
...
#endif
    if (ret < 0) {
		dprintk("%s read_data i2c_rxdata failed: %d\n", __func__, ret);
		return ret;
	}
...
//	dprintk("==work 1=\n");
	...
	else dprintk("data package read error\n");
//	dprintk("==work 2=\n");
//    	msleep(1);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	dprintk("==register_early_suspend =\n");
