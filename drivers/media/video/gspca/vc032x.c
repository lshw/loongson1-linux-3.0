...
static const struct v4l2_pix_format vc0321_mode[] = {
	{160, 120, V4L2_PIX_FMT_YVYU, V4L2_FIELD_NONE, /* poxxxx only */
		.bytesperline = 160,
		.sizeimage = 160 * 120 * 2,
	...
	{640, 480, V4L2_PIX_FMT_YVYU, V4L2_FIELD_NONE,
		...
		.priv = 0},
/*vish fixme: test*/
	{1280, 1024, V4L2_PIX_FMT_YVYU, V4L2_FIELD_NONE, /* po3130nc only */
		.bytesperline = 1280,
...
#if 1 /* 2.11.17 ms-win - to remove when set control done*/
...
/*	{0x00, 0xd5, 0x7c, 0xaa}, */
	...
	{0x00, 0xb0, 0x78, 0xaa},
	{0x00, 0x98, 0x02, 0xaa},	/* brightness */
	{0x00, 0x94, 0x25, 0xaa},	/* colors */
	{0x00, 0x95, 0x25, 0xaa},
	{0x00, 0x59, 0x68, 0xaa},	/* sharpness */
	{0x00, 0x44, 0x20, 0xaa},
	{0x00, 0x17, 0x50, 0xaa},	{0x00, 0x19, 0x50, 0xaa},
	{0x00, 0xd1, 0x3c, 0xaa},	/* autogain */
...
/*fixme:test*/
static const u8 po3130nc_InitSXGA50[][4] = {
	{0xb0, 0x4d, 0x00, 0xcc},	{0xb3, 0x01, 0x01, 0xcc},
	{0x00, 0x00, 0x50, 0xdd},	{0xb0, 0x03, 0x01, 0xcc},
	{0xb3, 0x00, 0x04, 0xcc},
	{0xb3, 0x00, 0x64, 0xcc},
...
static const u8 poxxxx_init_start_1[][4] = {
	{0xb3, 0x00, 0x04, 0xcc},
	{0x00, 0x00, 0x10, 0xdd},
	{0xb3, 0x00, 0x64, 0xcc},
	{0x00, 0x00, 0x10, 0xdd},
...
static const u8 poxxxx_init_start_2[][4] = {
	{0xb3, 0x05, 0x00, 0xcc},
	{0xb3, 0x06, 0x00, 0xcc},
	{0xb8, 0x00, 0x01, 0xcc},
...
static const u8 poxxxx_initQQVGA[][4] = {
	{0x00, 0x20, 0xbb, 0xaa},
	{0x00, 0x33, 0x38, 0xaa},
	{0x00, 0xbb, 0x0d, 0xaa},
	{0xb3, 0x22, 0x00, 0xcc},	/* change to 160x120 */
	{0xb3, 0x23, 0x78, 0xcc},
	{0xb3, 0x16, 0x00, 0xcc},
	{0xb3, 0x17, 0x9f, 0xcc},
	{0xb3, 0x02, 0xb3, 0xcc},
...
#if 0 //vish
/* initialize some global registers which are critical for integrated webcams */
static void vc0321_reset(struct gspca_dev *gspca_dev)
{
	reg_w(gspca_dev, 0xa0, 0x00, 0xb04d);
	reg_w(gspca_dev, 0xa0, 0x01, 0xb301);
	msleep(100);
	reg_w(gspca_dev, 0xa0, 0x01, 0xb003);
	msleep(100);
}
#endif
...
//		sensor = sd->sensor;
	...
	cam = &gspca_dev->cam;
	if (sd->bridge == BRIDGE_VC0321) {
		if (sd->sensor == SENSOR_PO3130NC) {
			cam->cam_mode = &vc0321_mode[1]; /* no 160x120 */
			cam->nmodes = ARRAY_SIZE(vc0321_mode) - 1;
		} else if (sd->sensor == SENSOR_POxxxx) { /* no 1280x1024 */
			cam->cam_mode = vc0321_mode;
			cam->nmodes = ARRAY_SIZE(vc0321_mode) - 1;
		} else {			/* 320x240 and 640x480 only */
			cam->cam_mode = &vc0321_mode[1];
			cam->nmodes = ARRAY_SIZE(vc0321_mode) - 2;
		}
	} else {
		switch (sensor) {
		case SENSOR_PO1200:
	...
	if (sd->bridge == BRIDGE_VC0321) {
		...
		reg_w(gspca_dev, 0x87, 0x00,
				0x0f00 | gspca_dev->usb_buf[2]);
		reg_r(gspca_dev, 0x8b, 0, 3);
		reg_w(gspca_dev, 0x88, 0x00,
				0x0200 | gspca_dev->usb_buf[2]);
		if (sd->sensor == SENSOR_POxxxx) {
...
#if 0
// vish
			/* full initialization to prevent integrated webcams
			 * to reset after 30s */
			if (gspca_dev->usb_err != 0)
				return gspca_dev->usb_err;
			msleep(500);
			usb_set_interface(gspca_dev->dev, gspca_dev->iface, 2);
			usb_exchange(gspca_dev, poxxxx_init_start_1);
			usb_exchange(gspca_dev, poxxxx_init_start_2);
			setgamma(gspca_dev);
			setbacklight(gspca_dev);
			setautogain(gspca_dev);
			usb_exchange(gspca_dev, poxxxx_init_start_3);
			usb_exchange(gspca_dev, poxxxx_initQVGA);
			msleep(1000);
			...
			reg_w(gspca_dev, 0xa0,
				...
				0xb35c);
			msleep(300);
			usb_exchange(gspca_dev, poxxxx_init_end_1);
...
/*fixme: back sensor only*/
	...
	case SENSOR_PO3130NC:
		...
		if (mode == 1)
			...
			init = po3130nc_InitSXGA50;	/* 1280x1024 */
...
/*fixme: i2c read 04 and 05*/
		usb_exchange(gspca_dev, poxxxx_init_end_1);
//vish
//	case SENSOR_PO3130NC:		2.11.17 ms-win
		setsharpness(gspca_dev);
		setwb(gspca_dev);
		setcolors(gspca_dev);
		setbrightness(gspca_dev);
...
//fixme: if needed
		...
		msleep(80);
	case SENSOR_PO3130NC:		/* 2.11.17 ms-win */
		reg_w(gspca_dev, 0x89, 0xffff, 0xfdff);	/* led on */
		break;
	}
	return gspca_dev->usb_err;
}
