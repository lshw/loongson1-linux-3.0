...
struct cam {
	...
	const struct framerates *mode_framerates; /* must have size nmodes,
						   * just like cam_mode */
	const struct v4l2_input *inputs; /* video inputs - size ninputs */
	v4l2_std_id std;		/* video standard */
	struct gspca_ctrl *ctrls;	/* control table - size nctrls */
					/* may be NULL */
	u32 bulk_size;		/* buffer size when image transfer by bulk */
	u32 input_flags;	/* value for ENUM_INPUT status flags */
	u8 nmodes;		/* size of cam_mode */
	u8 ninputs;		/* number of inputs */
...
typedef int (*cam_int_pkt_op) (struct gspca_dev *gspca_dev,
				u8 *data,
				int len);
typedef void (*cam_set_op) (struct gspca_dev *, int idx);
...
#define USB_BUF_SZ 64
	...
	u8 curr_input;			/* current video input */
	v4l2_std_id curr_std;		/* current video standard */
