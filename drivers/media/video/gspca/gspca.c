			...
			struct ep_tb_s *ep_tb)
{
	...
	/* get the requested bandwidth and start at the highest altsetting */
...
static int vidioc_g_std(struct file *filp, void *priv,
			v4l2_std_id *parm)
{
	struct gspca_dev *gspca_dev = priv;

	*parm = gspca_dev->curr_std;
	return 0;
}
			...
			v4l2_std_id *parm)
{
	...
	if (!(*parm & gspca_dev->cam.std)
	 || gspca_dev->sd_desc->set_misc == NULL)
		return -EINVAL;
	gspca_dev->curr_std = *parm;
	gspca_dev->usb_err = 0;
	gspca_dev->sd_desc->set_misc(gspca_dev, MISC_STD);
	return gspca_dev->usb_err;
}

static int vidioc_querystd(struct file *filp, void *priv,
			v4l2_std_id *parm)
{
	struct gspca_dev *gspca_dev = priv;

	*parm = gspca_dev->cam.std;
	return 0;
}
				...
				struct v4l2_input *input)
{
	...
	i = input->index;
	if (gspca_dev->cam.ninputs == 0) {	/* only input */
		if (i != 0)
			return -EINVAL;
		input->type = V4L2_INPUT_TYPE_CAMERA;
		input->status = gspca_dev->cam.input_flags;
		strlcpy((char *) input->name, gspca_dev->sd_desc->name,
			sizeof input->name);
		return 0;
	}
	if (i >= gspca_dev->cam.ninputs)
		return -EINVAL;
	memcpy(input, &gspca_dev->cam.inputs[i], sizeof *input);
	input->index = i;
...
static int vidioc_s_input(struct file *file, void *priv, unsigned int i)
{
	...
	if (i >= gspca_dev->cam.ninputs && i != 0)
		return -EINVAL;
	gspca_dev->curr_input = i;
	gspca_dev->usb_err = 0;
	if (gspca_dev->sd_desc->set_misc)
		gspca_dev->sd_desc->set_misc(gspca_dev, MISC_INPUT);
	return gspca_dev->usb_err;
}
