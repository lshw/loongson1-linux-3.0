...
static int ls1x_wdt_probe(struct platform_device *pdev)
{
	...
	wdt = kzalloc(sizeof(struct wdt_ls1x), GFP_KERNEL);
	if (!wdt) {
	...
	wdt_clock = clk_get(&pdev->dev, "apb");
	if (IS_ERR(wdt_clock)) {
...
static int __devexit ls1x_wdt_remove(struct platform_device *pdev)
{
...
static struct platform_driver ls1x_wdt_driver = {
	...
	.remove		= __devexit_p(ls1x_wdt_remove),
...
static int __init ls1x_wdt_init(void)
{
	return platform_driver_register(&ls1x_wdt_driver);
}

static void __exit ls1x_wdt_exit(void)
{
	platform_driver_unregister(&ls1x_wdt_driver);
}

module_init(ls1x_wdt_init);
module_exit(ls1x_wdt_exit);
