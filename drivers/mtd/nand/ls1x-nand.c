...
//	kfree(info->drcmr_dat);
	dma_free_coherent(&pdev->dev, MAX_BUFF_SIZE, (dma_addr_t *)info->drcmr_dat, info->drcmr_dat_phys);
	dma_free_coherent(&pdev->dev, info->data_buff_size, info->data_buff, info->data_buff_phys);
	kfree(mtd);
...
static int ls1b_nand_suspend(struct platform_device *pdev, pm_message_t pm)
{
