...
static int probe (struct pci_dev *pdev, const struct pci_device_id *my_pci_id) 
{
  ...
  if (!synopGMACMappedAddr){
      TR0 ("ioremap_nocache failed with addrrss %p\n",  synopGMACMappedAddr);
  }
  
  TR ("Physical address = %08x\n", the_register_resource_base);
  TR ("Remapped address = %p\n",  synopGMACMappedAddr);
  
    /*Check if region is already locked by any other driver ? */ 
    if (check_mem_region ( (resource_size_t)synopGMACMappedAddr, synopGMACMappedAddrSize))
    {
    ...
    request_mem_region ((resource_size_t) synopGMACMappedAddr, synopGMACMappedAddrSize,"synopGMACmemory");
    TR ("Requested memory region for synopGMACMappedAddr = 0x%p\n",  synopGMACMappedAddr);
  
    /*Now pci interface is ready. Let give this information the the HOST module */ 
	if((synopGMACadapter=synopGMAC_init_network_interface(&pdev->dev,synopGMACMappedAddr,synopGMACMappedAddrSize,pdev->irq)))
	pci_set_drvdata(pdev,synopGMACadapter);
...
{
synopGMACPciNetworkAdapter *adapter=pci_get_drvdata(dev);
