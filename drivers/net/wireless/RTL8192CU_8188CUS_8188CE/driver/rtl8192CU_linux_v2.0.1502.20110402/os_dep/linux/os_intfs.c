...
uint eap_data_rate=0;
module_param(eap_data_rate, uint, 0644);
...
u8 rtw_init_default_value(_adapter *padapter)
{
	...
	psecuritypriv->sw_encrypt=1; // before connected to AP and enable_hw_enc_timer timeout, enforce software enc
	#else
	psecuritypriv->sw_encrypt=pregistrypriv->software_encrypt;
	#endif
	printk("initial psecuritypriv->sw_encrypt:%d\n",psecuritypriv->sw_encrypt);
...
_func_enter_;
	...
	#ifdef ENABLE_HW_ENC_TIMER
	_init_timer(&(padapter->securitypriv.enable_hw_enc_timer), padapter->pnetdev, rtw_enable_hw_enc_handler, padapter);
	#endif
...
*/
...
#ifdef ENABLE_HW_ENC_TIMER
	_set_timer(&padapter->securitypriv.enable_hw_enc_timer, 10000);
	while(1)
	{
		_cancel_timer(&padapter->securitypriv.enable_hw_enc_timer, &bcancelled);
		if (bcancelled == _TRUE)
			break;
	}
	RT_TRACE(_module_os_intfs_c_,_drv_info_,("\n drv_halt:cancel enable_hw_enc_timer! \n"));
#endif
	...
	DBG_871X("DBG_POWER_SAVING %s:%d set pwr_state_check_timer %d\n", __FUNCTION__, __LINE__, 10000);
	#endif
	 while(1)
   	 {
	       _cancel_timer(&padapter->pwrctrlpriv.pwr_state_check_timer, &bcancelled);
		#ifdef DBG_POWER_SAVING
		DBG_871X("DBG_POWER_SAVING %s:%d cancel pwr_state_check_timer\n", __FUNCTION__, __LINE__);
		#endif
...
static int netdev_open(struct net_device *pnetdev)
{
	...
	{	
		...
		#ifdef DBG_POWER_SAVING
		DBG_871X("DBG_POWER_SAVING %s:%d set pwr_state_check_timer %d\n", __FUNCTION__, __LINE__,padapter->pwrctrlpriv.pwr_state_check_inverval);
		#endif
	}
