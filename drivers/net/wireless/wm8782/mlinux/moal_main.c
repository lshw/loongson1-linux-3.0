...
/** Driver version */
char driver_version[] =
    "USB8782-%s-M2614" MLAN_RELEASE_VERSION "-GPL" "-(" "FP" ")"
//    "USB8782-%s-M2614" MLAN_RELEASE_VERSION "-GPL" "-(" "FP" FPNUM ")"	//lxy
#ifdef	DEBUG_LEVEL2
    "-dbg"
#endif
    " ";
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)	//lxy
 ...
 *  @param info    A pointer to mlan_debug_info structure
 *
 *  @return 	   N/A
 */
void
woal_display_debug_info(moal_private * priv, mlan_debug_info * info)
{
    int i;

    ENTER();

    PRINTM(MERROR, "num_tx_timeout = %ld\n", priv->num_tx_timeout);
...
woal_tx_timeout(struct net_device *dev)
{
    ...
    if (!woal_get_debug_info(priv, MOAL_CMD_WAIT, &info)) {
        woal_display_debug_info(priv, &info);
    }
