...
static int __devinit ls1x_mci_probe(struct platform_device *pdev)
{
	struct ls1x_mci_host *host;
	struct mmc_host	*mmc;
	int ret;
...
static int __devexit ls1x_mci_remove(struct platform_device *pdev)
{
	struct mmc_host		*mmc  = platform_get_drvdata(pdev);
	struct ls1x_mci_host	*host = mmc_priv(mmc);
...
static int ls1x_mci_suspend(struct device *dev)
{
...
static int ls1x_mci_resume(struct device *dev)
{
	struct mmc_host *mmc = platform_get_drvdata(to_platform_device(dev));
...
static const struct dev_pm_ops ls1x_mci_pm = {
	.suspend	= ls1x_mci_suspend,
	.resume		= ls1x_mci_resume,
};

#define ls1x_mci_pm_ops &ls1x_mci_pm
#else /* CONFIG_PM */
#define ls1x_mci_pm_ops NULL
#endif /* CONFIG_PM */
...
static struct platform_driver ls1x_mci_driver = {
	...
	.remove		= __devexit_p(ls1x_mci_remove),
	.shutdown	= ls1x_mci_shutdown,
};

static int __init ls1x_mci_init(void)
{
	return platform_driver_register(&ls1x_mci_driver);
}

static void __exit ls1x_mci_exit(void)
{
	platform_driver_unregister(&ls1x_mci_driver);
}

module_init(ls1x_mci_init);
module_exit(ls1x_mci_exit);
